...
cp "$script_dir/packages/sources.list" /etc/apt/sources.list

/bin/echo -e '\033[1;32m==>\033[0;1m Install apt-offline\033[m'
PATH="$PATH:/usr/sbin" dpkg -i $(find "$script_dir/packages/apt-offline/" | grep -P '\.deb$')

/bin/echo -e '\033[1;32m==>\033[0;1m Install update.zip\033[m'
apt-offline install "$script_dir/packages/update.zip"

/bin/echo -e '\033[1;32m==>\033[0;1m Install upgrade.zip\033[m'
apt-offline install "$script_dir/packages/upgrade.zip"
...
user_cmd 'test -e camp_sim/' || user_cmd "tar xf '$script_dir/packages/camp_sim.tgz'"
